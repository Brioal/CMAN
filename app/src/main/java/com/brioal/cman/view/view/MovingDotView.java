...
import com.brioal.cman.view.exceptions.SizeNotDeterminedException;
import com.brioal.cman.view.listener.OnAnimatorChangeListener;
...
public class MovingDotView extends ViewGroup {
    ...
    private void initObtainStyled(Context context, AttributeSet attrs) {
        ...
        mDotColor = array.getColor(R.styleable.MovingDotView_md_dot_color, getResources().getColor(R.color.colorFirst));
        ...
        mBtnTextColor = array.getColor(R.styleable.MovingDotView_md_btn_text_color, getResources().getColor(R.color.colorFirst));
...
//            return;
//        }
        mCenterDot.setStatue("正在连接");
        startAnimation(0, 1, toProgress);
        isCleaned = true;
    }

    //连接成功
    public void connectedDone() {
        if (mCenterDot == null) {
            return;
        }
        mCenterDot.setStatue("连接成功");
